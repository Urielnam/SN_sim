...
import numpy as np
import os
from new_calc_multiscale import compute_multiscale_variety
from micro_agents import MicroAgent
...
def pad_matrix_to_dim(matrix, target_dim):
    """
    Cyclically duplicates columns of a matrix until it reaches target_dim.
    This preserves joint entropy while aligning scales.
    """
    current_dim = matrix.shape[1]

    # If it's already the right size, or completely empty, return it as-is
    if current_dim == 0 or current_dim == target_dim:
        return matrix

    # Tile the matrix horizontally enough times to cover the target_dim
    tiles_needed = (target_dim // current_dim) + 1
    tiled_matrix = np.tile(matrix, (1, tiles_needed))

    # Truncate it to exactly the target dimension
    padded_matrix = tiled_matrix[:, :target_dim]
...
def get_restricted_view(self, global_state, agent_id):
    ...
    visible_agents = [val for val, mask in zip(agent_states, p2p_mask) if mask == 1]
    visible_metrics = [val for val, mask in zip(system_metrics, s2p_mask) if mask == 1]

    return visible_agents + visible_metrics
...
def get_historical_matrices(self, agent_id, T_window):
    """
    Extracts the last T_window states and builds strict N-dimensional
    historical matrices for the visible agents and system metrics.
    """

    # Fetch up to the last T_window snapshots from the context
    snapshots = self.ctx.state_snapshots[-T_window:]
    ...
    for snap in snapshots:
        # Reconstruct the true state for this specific tick
        agents_full = [snap["num_iiots"], snap["bus_flow"], snap["edge_flow"], snap["scada_flow"]]
        system_full = [snap["queue_len"], snap["success_rate"], snap["avg_latency"]]

        # Filter: Keep ONLY the variables this agent is allowed to see
        visible_agents = [val for val, mask in zip(agents_full, p2p_mask) if mask == 1]
        visible_system = [val for val, mask in zip(system_full, s2p_mask) if mask == 1]

        agent_history.append(visible_agents)
        system_history.append(visible_system)

    return np.array(agent_history), np.array(system_history)
...
class MaskedRandomStrategy(BaseStrategy):
    ...
    def agent_loop(self, agent_id):
        ...
        while True:
            ...
            global_state = get_global_state(self)

            # 2. Filter it through the matrices
            restricted_view = get_restricted_view(self, global_state, agent_id)
            ...
            # 4. Calculate Subjective Multiscale Variety
            # We only calculate if the buffer is full enough to provide a meaningful distribution
            if len(self.ctx.state_snapshots) >= T_window:
                agent_mat, sys_mat = get_historical_matrices(self, agent_id, T_window)

                N_agent = agent_mat.shape[1]
                N_sys = sys_mat.shape[1]

                # We only proceed if the agent has visibility of at least something
                if N_agent > 0 and N_sys > 0:
                    # Find the maximum dimension to align to
                    target_N = max(N_agent, N_sys)

                    # Pad the matrices dynamically
                    aligned_agent_mat = pad_matrix_to_dim(agent_mat, target_N)
                    aligned_sys_mat = pad_matrix_to_dim(sys_mat, target_N)

                    # Calculate Multiscale Variety on aligned matrices
                    Q_a, D_a, V_a = compute_multiscale_variety(aligned_agent_mat)
                    Q_s, D_s, V_s = compute_multiscale_variety(aligned_sys_mat)

                    # Uncomment to debug and verify the scales match perfectly
                    # print(f"Time {self.env.now} | Agent {agent_id} | Aligned N={target_N}")
                    # print(f"   Agent V(k): {V_a}")
                    # print(f"   System V(k): {V_s}")

                    # Calculate Requisite Variety Deficit (The Loss J)
                    # V_s is System (Environment) Complexity -> V_X
                    # V_a is Agent Complexity -> V_Y
                    J_loss = 0.0
                    for k in range(1, target_N + 1):
                        # Calculate deficit at scale k
                        deficit = max(0, V_s[k] - V_a[k])
                        J_loss += deficit

                    # Log it to the console to observe
                    print(f"Time {self.env.now} | Agent {agent_id} | Deficit J: {J_loss:.4f} bits")


# ==========================================
# Masked RL strategy (information stream design)
# ==========================================
class MaskedRLStrategy(BaseStrategy):
    """
    Multi-Agent Reinforcement Learning Strategy driven by Multiscale Variety.
    Agents adapt online, seeking to minimize the deficit between their internal
    complexity (V_a) and the environment's complexity (V_s) based on Ashby's Law.
    """


    def __init__(self, ctx, bus, edge, scada, p2p_matrix=None, s2p_matrix=None):
        # --- DEBUGGING TRAP ---
        print(f"DEBUG: p2p_matrix type is {type(p2p_matrix)}")
        print(f"DEBUG: p2p_matrix value is {p2p_matrix}")
        super().__init__(ctx, bus, edge, scada, p2p_matrix, s2p_matrix)

        # Hyperparameters from Config
        self.stack_size = self.ctx.config.frame_stack_size
        self.stride = self.ctx.config.training_stride
        self.max_window = self.ctx.config.max_history_window
        self.alpha = self.ctx.config.alpha_penalty

        # Initialize 4 Independent Micro-Agents (IPPO/IDQN approach)
        self.agents = {}
        self.frame_stacks = {}
        self.recent_transitions = {0: [], 1: [], 2: [], 3: []}  # Temporarily holds (s, a) until reward is calculated

        for i in range(4):
            # Calculate input dimension dynamically based on the mask

            num_visible_vars = sum(self.p2p_matrix[i]) + sum(self.s2p_matrix[i])
            input_dim = num_visible_vars * self.stack_size

            # Action dim is uniformly 3 for all agents to simplify architecture
            self.agents[i] = MicroAgent(input_dim=input_dim, action_dim=3)

            # Initialize empty frame stacks with zeros
            self.frame_stacks[i] = deque([np.zeros(num_visible_vars)] * self.stack_size, maxlen=self.stack_size)

    def setup(self):
        """Starts the asynchronous SimPy clock for agents."""

        for i in range(4):
            self.env.process(self.agent_loop(agent_id=i))

        # We completely removed the global learning_stride_loop.
        # Agents are now 100% autonomous.

    def agent_loop(self, agent_id):
        """The fully autonomous, decentralized execution and learning loop."""
        # Calculate how many variables this agent actually sees to size its memory stack
        # (Using standard Python 'sum' on the 1/0 masks)

        num_visible_vars = sum(self.p2p_matrix[agent_id]) + sum(self.s2p_matrix[agent_id])

        # Initialize the stack with empty zeros so the neural network doesn't crash on tick 1
        stack_size = 4  # (Or use sim_config.frame_stack_size)
        self.frame_stacks[agent_id] = deque([np.zeros(num_visible_vars)] * stack_size, maxlen=stack_size)

        while True:
            yield self.env.timeout(1.0)

            # --- 1. OBSERVE & ACT ---
            snap = self.ctx.state_snapshots[-1]

            global_state = get_global_state(self)

            restricted_view = get_restricted_view(self, global_state, agent_id)

            # Push the new observation into the deque
            self.frame_stacks[agent_id].append(restricted_view)

            # Flatten the deque of 4 arrays into a single 1d numpy array
            stacked_state = np.concatenate(self.frame_stacks[agent_id])

            # Pass the flattened 4-timestep history to the agent
            action = self.agents[agent_id].act(stacked_state)

            # --- 2. EXECUTE PHYSICAL ACTION ---
            if agent_id == 0:
                if action == 1:
                    self.add_iiot()
                elif action == 2 and len(self.ctx.iiot_list) > 1:
                    self.remove_sensor()
            else:
                modifier = 0 if action == 0 else (1 if action == 1 else -1)
                target = "bus" if agent_id == 1 else ("edge" if agent_id == 2 else "scada")
                if modifier != 0: self.modify_flow_rate(target, modifier)

            # Store transition temporarily (Reward is unknown right now)
            self.recent_transitions[agent_id].append({
                "state": stacked_state,
                "action": action
            })

            # --- 3. THE AUTONOMOUS ADAPTATION CYCLE ---
            # The agent independently decides to reflect and learn every 'stride' ticks
            if self.env.now > 0 and self.env.now % self.stride == 0:

                T_current = len(self.ctx.state_snapshots)
                window_size = min(self.max_window, T_current)

                # Only learn if there is enough historical data
                if window_size >= 10:
                    # 1. Fetch current success for reward shaping directly from ctx
                    current_success = self.ctx.state_snapshots[-1]["success_rate"]

                    # 2. Call the helper correctly (it fetches its own history and masks internally)
                    agent_hist, sys_hist = get_historical_matrices(self, agent_id, window_size)

                    J_loss = 0.0

                    if agent_hist.shape[1] > 0 and sys_hist.shape[1] > 0:
                        target_N = max(agent_hist.shape[1], sys_hist.shape[1])

                        aligned_a = pad_matrix_to_dim(agent_hist, target_N)
                        aligned_s = pad_matrix_to_dim(sys_hist, target_N)

                        _, _, V_a = compute_multiscale_variety(aligned_a)
                        _, _, V_s = compute_multiscale_variety(aligned_s)

                        # Deficit J = sum(max(0, V_X(k) - V_Y(k)))
                        for k in range(1, target_N + 1):
                            J_loss += max(0, V_s[k] - V_a[k])

                    # Reward shaping: Physical success minus structural deficit penalty
                    final_reward = current_success - (self.alpha * J_loss)

                    # Push local transitions into the agent's memory
                    for trans in self.recent_transitions[agent_id]:
                        self.agents[agent_id].memory.push(
                            state=trans["state"],
                            action=trans["action"],
                            reward=final_reward,
                            next_state=trans["state"]  # Approximated
                        )

                    self.recent_transitions[agent_id].clear()

                    # The agent triggers its own internal backpropagation
                    self.agents[agent_id].learn()

                    # (Be sure to delete the old learning_stride_loop function entirely)
                    # 5. Push transitions into Replay Buffer and Learn
                    for trans in self.recent_transitions[agent_id]:
                        # Next state is handled loosely here for continuous environments
                        self.agents[agent_id].memory.push(
                            state=trans["state"],
                            action=trans["action"],
                            reward=final_reward,
                            next_state=trans["state"]  # Approximated next state
                        )

                    # Clear the temporary transition list
                    self.recent_transitions[agent_id].clear()

                    # 6. Trigger PyTorch Backpropagation
                    self.agents[agent_id].learn()